...
use forge_core::model::{
    AssertionDef, Check, Extractor, ExtractorSource, ExtractScope, NumberOp, StringOp, ValueOp,
};
...
fn generate_then_evaluate_all_pass_on_the_same_response() {
    ...
    assert!(matches!(generated.first(), Some(Check::StatusCode { op: NumberOp::Eq, value: 200 })));
    assert!(matches!(generated.last(), Some(Check::ResponseTimeBelow { .. })));
    assert!(generated.iter().any(|c| matches!(c, Check::ContentType { value } if value == "application/json")));
    ...
    for outcome in &outcomes {
        assert!(outcome.passed, "expected {} to pass, got {:?}", outcome.summary, outcome.message);
    }
}
...
fn generation_uses_bracket_notation_for_dotted_key() {
    ...
    assert!(has_bracket_path, "expected a bracket-quoted path for the dotted key");
}
...
fn manual_assertions_against_fixture() {
    ...
    let checks = vec![
        Check::StatusCode { op: NumberOp::Eq, value: 404 },
        Check::StatusClass { class: 4 },
        Check::Header { name: "x-request-id".into(), op: StringOp::Matches, value: r"^req-\d+$".into() },
        Check::JsonPath {
    ...
    for check in &checks {
        ...
        assert!(outcome.passed, "{} failed: {:?}", outcome.summary, outcome.message);
    }

    // And one that should fail, with a precise message.
    let failing = Check::StatusCode { op: NumberOp::Eq, value: 200 };
    let outcome = evaluate(&failing, &res);
    assert!(!outcome.passed);
    assert_eq!(outcome.message.as_deref(), Some("expected status == 200, got 404"));
}
...
fn extraction_from_fixture() {
    ...
    let extractors = vec![
        Extractor {
            source: ExtractorSource::JsonPath { expr: "$.pricing.currency".into() },
            var: "currency".into(),
            scope: ExtractScope::Runtime,
            enabled: true,
        },
        Extractor {
            source: ExtractorSource::Header { name: "x-request-id".into() },
            var: "request_id".into(),
            scope: ExtractScope::Runtime,
            enabled: true,
        },
        Extractor {
            ...
            source: ExtractorSource::Regex { pattern: r#""name":\s*"(\w+)""#.into(), group: 1 },
            var: "item_name".into(),
