...
/// Import a Forge JSON bundle or a cURL script exported by Forge. Bundle
...
fn render_curl(bundle: &ForgeBundle) -> Result<String, String> {
    ...
    let mut output = String::from(
        "#!/usr/bin/env sh\n# Generated by Forge. Import this script to restore its full request metadata.\n",
    );
...
fn parse_bundle(text: &str) -> Result<ForgeBundle, String> {
    if text.trim_start().starts_with('{') {
        return serde_json::from_str(text)
            .map_err(|error| format!("invalid Forge bundle: {error}"));
    }
    ...
    for line in text.lines() {
        ...
        if inside {
            let chunk = line
                ...
                .strip_prefix('#')
                .map(str::trim)
                .ok_or_else(|| "invalid embedded Forge bundle".to_string())?;
            encoded.push_str(chunk);
        }
    }
    if inside || encoded.is_empty() {
        return Err("file is neither a Forge JSON bundle nor a Forge cURL export".to_string());
    }
    let json = BASE64
        .decode(encoded)
        .map_err(|error| format!("invalid embedded Forge bundle: {error}"))?;
    serde_json::from_slice(&json).map_err(|error| format!("invalid embedded Forge bundle: {error}"))
}
